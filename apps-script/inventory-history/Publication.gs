...
function pccHistPublish_(deadline){
 ...
 if(!stateId||props.getProperty('PCC_HIST_PUBLISHED_STATE')===stateId)return;
 ...
 for(var i=0;i<state.loads.length;i++){
  ...
  if(batch.inventoryComplete){
   var detail={schema:1,cutoff:batch.cutoff,rows:PccHistoryModel.groupStock(batch.inventory)};
